...
CRoom_2x1::CRoom_2x1(CWindow* window, CMap* collisionMap, std::vector<CRoom*>* collisionRoom,
...
void CRoom_2x1::check_up(CRoom* roomToCheck, int pixelCheck)
{
...
void CRoom_2x1::check_down(CRoom* roomToCheck, int pixelCheck)
{
	// null means not found
...
void CRoom_2x1::check_left(CRoom* roomToCheck, int pixelCheck)
{
	// null means not found
	if (m_pRoom_11_left == NULL || m_pRoom_12_left == NULL)
	{
		...
		if (roomToCheck->collision(&pointToCheck) == true)
		{
			// this is a 2x1 room, so all left's are the same, and should point to the same room
...
void CRoom_2x1::check_right(CRoom* roomToCheck, int pixelCheck)
{
	// null means not found
	if (m_pRoom_21_right == NULL || m_pRoom_22_right == NULL)
	{
		...
		if (roomToCheck->collision(&pointToCheck) == true)
		{
			// this is a 2x1 room, so all right's are the same, and should point to the same room
...
SCoords2<int> CRoom_2x1::whichSubRoom(SCoords2<int>* pPoint)
{
